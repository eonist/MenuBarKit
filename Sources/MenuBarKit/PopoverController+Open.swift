...
extension MBKPopoverController {
    ...
    func openPopover() {
        ...
        // small size. The ScrollView's .frame(height:) guard in the host view
        // returns nil when scrollViewHeight is 0, so SwiftUI measures the content
        // unconstrained on the next pass — the grow-from-stale-height step is
        // eliminated because the popover is never seeded with a stale height.
        //
        // DEAD-BAND SIDE EFFECT:
        // applyContentSize's 1pt dead-band guard
        //   (abs(old.height - new.height) > 1)
        // was suppressing the pre-show write when the post-close SwiftUI flush
        // happened to write back the same value. The reset to (minWidth, 100)
        // clears that dead-band unconditionally, ensuring the first real
